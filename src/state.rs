...
mod test {
    ...
    #[test]
    fn test_calc_pool_deposit_amount_stake_total_zero() {
        let stake_pool = StakePool::default();
        let actual = stake_pool.calc_pool_deposit_amount(360);

        assert_eq!(actual, Some(360u64));
    }

    #[test]
    fn test_calc_pool_deposit_amount_stake_total_increase() {
        let mut stake_pool = StakePool::default();
        stake_pool.stake_total = 200u64;
        stake_pool.pool_total = 160u64;
        let actual = stake_pool.calc_pool_deposit_amount(360);

        assert_eq!(actual, Some(288u64));
    }

    #[test]
    fn test_calc_pool_deposit_amount_stake_total_larger_than_deposit() {
        let mut stake_pool = StakePool::default();
        stake_pool.stake_total = 200u64;
        stake_pool.pool_total = 160u64;
        let actual = stake_pool.calc_pool_deposit_amount(100);

        assert_eq!(actual, Some(80u64));
    }

    #[test]
    fn test_calc_pool_deposit_amount_pool_total_larger_than_stake_total() {
        let mut stake_pool = StakePool::default();
        stake_pool.stake_total = 200u64;
        stake_pool.pool_total = 260u64;
        let actual = stake_pool.calc_pool_deposit_amount(100);

        assert_eq!(actual, Some(130u64));
    }

    #[test]
    fn test_calc_lamports_amount() {
        let mut stake_pool = StakePool::default();
        stake_pool.stake_total = 200u64;
        stake_pool.pool_total = 160u64;
        let actual = stake_pool.calc_lamports_amount(100);

        assert_eq!(actual, Some(125u64));
    }
}
