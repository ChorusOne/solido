...
pub fn process_stake_deposit(
    ...
    let (validator_key, validator) = lido.validators.get_mut(&accounts.validator_stake_pool_stake_account.key)?;
    ...
    let stake_account_seed = validator.stake_accounts_seed_end.to_le_bytes();
...
pub fn process_deposit_active_stake_to_pool(
    ...
    let stake_account_seed = validator.stake_accounts_seed_begin.to_le_bytes();
    let stake_account_bump_seed = [stake_addr_bump_seed];
    let solido_address_bytes = accounts.lido.key.to_bytes();
